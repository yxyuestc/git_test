...
// YoloV5::YoloV5(const YoloV5 &obj)
// 	: m_bmContext(obj.m_bmContext)
// 	, m_bmNetwork(obj.m_bmNetwork)
// 	, m_resized_imgs(obj.m_resized_imgs)
// 	, m_converto_imgs(obj.m_converto_imgs)
// 	, m_confThreshold(obj.m_confThreshold)
// 	, m_nmsThreshold(obj.m_nmsThreshold)
// 	, m_objThreshold(obj.m_objThreshold)
// 	, m_class_names(obj.m_class_names)
// 	, m_class_num(obj.m_class_num)
// 	, m_net_h(obj.m_net_h)
// 	, m_net_w(obj.m_net_w)
// {}; 
...
void YoloV5::drawPred_red(int classId, float conf, int left, int top, int right, int bottom, cv::Mat& frame)   // Draw the predicted bounding box
{
  ...
  if ((int)m_class_names.size() >= m_class_num) {
    label = this->m_class_names[classId] ;
  }else{
...
void YoloV5::drawPred_green(int classId, float conf, int left, int top, int right, int bottom, cv::Mat& frame)   // Draw the predicted bounding box
{
  //Draw a rectangle displaying the bounding box
...
void YoloV5::drawPred_blue(int classId, float conf, int left, int top, int right, int bottom, cv::Mat& frame)   // Draw the predicted bounding box
{
  //Draw a rectangle displaying the bounding box
  cv::rectangle(frame, cv::Point(left, top), cv::Point(right, bottom), cv::Scalar(255, 0, 0), 3);
  std::string label;
  if ((int)m_class_names.size() >= m_class_num) {
    label = this->m_class_names[classId];
  }else{
  ...
  cv::Size labelSize = getTextSize(label, cv::FONT_HERSHEY_SIMPLEX, 0.5, 1, &baseLine);
  top = std::max(top, labelSize.height);
  cv::putText(frame, label, cv::Point(left, top), cv::FONT_HERSHEY_SIMPLEX, 0.75, cv::Scalar(255, 0, 0), 1);
}
